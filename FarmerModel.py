...
def farmerEnvToOnehot(cards,llc):
    Env2IdxMap = {3:0,4:1,5:2,6:3,7:4,8:5,9:6,10:7,11:8,12:9,13:10,14:11,17:12,20:13,30:14}
    cards = [Env2IdxMap[i] for i in cards]
    llcs = [Env2IdxMap[c] for c in llc]
    Onehot = torch.zeros((7,15))
    for i in range(0, 15):
        Onehot[:cards.count(i),i] = 1
    return Onehot
...
if os.path.exists("./landlord_down_weights.pkl"):
    if torch.cuda.is_available():
        ...
        Nets["down"].load_state_dict(torch.load("./landlord_down_weights.pkl", map_location=torch.device("cpu")))
    Nets["down"].eval()
...
#     y = net(x)[0].item()
#     return y * 100
def predict(cards, llc, type):
    net = Nets[type]
    x = torch.flatten(farmerEnvToOnehot(cards, llc))
    y = net(x)[0].item()
    return y * 100